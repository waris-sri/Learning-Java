...
class MedianSortedArrays {
    public static double findMedianSortedArrays(int[] nums1, int[] nums2) {
        ...
        while (left <= right) {
            ...
            // CASE 2: The elements on the left of i is larger than j
                ...
                int leftHalfMax;
                if (i == 0)
                    leftHalfMax = nums2[j - 1]; // CASE #3.1
                else if (j == 0)
                    leftHalfMax = nums1[i - 1]; // CASE #3.2
                ...
                // 3.3.2: m<n, all elements of nums2 are greater than nums1
                    ...
                    leftHalfMax = Math.max(nums1[i - 1], nums2[j - 1]);

                // If the length of the large array is odd, the median is the maximum value of
                ...
                int rightHalfMin;
                if (i == m)
                    rightHalfMin = nums2[j]; // CASE #3.4
                else if (j == n)
                    rightHalfMin = nums1[i]; // CASE #3.5
                else
                    rightHalfMin = Math.min(nums1[i], nums2[j]); // CASE #3.6

                // If the length of the large array is even, the median is the average of the
                // leftHalfMax and rightHalfMin
                if ((m + n) % 2 == 0)
    ...
    public static void main(String[] args) {
        ...
        int[] nums2 = new int[] { 8, 9, 9, 10, 11, 14 };
